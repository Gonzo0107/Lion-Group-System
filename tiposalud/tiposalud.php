...
<a href="tiposaludinsertar.php" class="btn btn-success ">Insertar Tipo de salud</a>
</div>
</div>
<br>

<div class="container px-4 border border-secondary shadow p-3 mb-5 bg-white rounded pb-5">
	<h4>Buscar Tipo Salud</h4>
		...
		<div class="form-group col-md-4">
			...
			if (isset($_GET['enviar'])) {
				...
				$consulta = $bd->query("SELECT * FROM tipo_salud  WHERE id_salud = '$busqueda' OR nombre_salud = '$busqueda' OR descrpcion = '$busqueda' ");
				$resultado = $consulta->fetchALL(PDO::FETCH_OBJ);

				foreach ($resultado as $resul) {
					?>
					<li><?php echo "$resul->id_salud | $resul->nombre_salud | $resul->descrpcion  "?></li>
...
<table class="table">
				...
				$datos = $sentencia->fetchALL(PDO::FETCH_OBJ);
					...
					foreach ($datos as $dato) {
						?>
							<tr>
								<td><?php echo $dato->id_salud; ?></td>
								<td><?php echo $dato->nombre_salud; ?></td>
								<td><?php echo $dato->descrpcion; ?></td>
								<td><a type="submit" class="btn btn-primary" href="editar.php?id=<?php echo $dato->id_salud; ?>" >Editar</td>
								<td><a type="submit" class="btn btn-danger"  href="eliminar.php?id=<?php echo $dato->id_salud; ?>">Eliminar</td>
